 ...
 * Program: Program 5 - encrypt.cpp
 * Due Date: April 18, 2020
 * Purpose: to encrypt content from a given input file and output to a separate given output file
 */
...
/*Get file names and shift amount*/
    ...
    //Check if shift key is in bounds
    if (key < 1 || key > 255)
    {
        cout << "Error: shift key out of bounds (1 - 255)" << endl;
        exit(2);
    }
...
/*Flip key for second shift*/
    ...
    //Place key at start of output file
    output << key << "\n";

    //Get first letter from the input file
    letter = input.get();

/*Read characters from input file and place in output file*/
    while (!input.eof())
    {
        //Shift each character by shift amount
        letter += key;
